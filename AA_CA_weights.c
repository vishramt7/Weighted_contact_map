...
char atom_no_array[5], residue_no_array[5], x_array[15], y_array[15], z_array[15];
	...
	while (*str_point)
	{
		...
		if ((a >= 6) && (a <= 10))
			atom_no_array[a-6] = *str_point;
		
		if ((a >= 22) && (a <= 25)) 
			residue_no_array[a-22] = *str_point;
		
		if ((a >= 30) && (a <= 37))
			x_array[a-30] = *str_point;
		
		if ((a >= 38) && (a <= 45))
			y_array[a-38] = *str_point;
		
		if ((a >= 46) && (a <= 53))
			z_array[a-46] = *str_point;
	str_point++;	
	a++;
	}
...
char atom[6];                   /* This are the first 6 characters in pdb */
...
FILE *pdb = fopen (argv[1],"r");
	...
	MAP[k] = line;
	while (fgets (strg, sizeof(strg), pdb) != NULL)
	{
	last_element = strlen(strg) - 1;

		if (strg[last_element] == '\n')
			strg[last_element] = '\0';	
